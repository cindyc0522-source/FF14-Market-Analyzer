    ...
    if target_item and st.button("啟動深度分析 🚀", type="primary"):
        ...
        try:
            ...
            if not recipes:
                ...
                if 'GilShopItem' in links:
                    source_msgs.append("💰 NPC 商店")

                if 'SpecialShop' in links:
                    source_msgs.append("🔄 特殊商店/兌換")

                if 'InstanceContent' in links:
                    source_msgs.append("⚔️ 副本掉落")

                if 'Achievement' in links:
                    source_msgs.append("🏆 成就獎勵")
...
for ing in ingredients:
    ...
    details.append({
        ...
        "市場": market_text
                ...
                m1.metric(f"成品最低單價 ({world})", f"{price} G")
        ...
        if st.form_submit_button("批次掃描 🚀"):
            ...
            for idx, name in enumerate(items):
                ...
                if name in name_to_item_id:
                    ...
                    p, _ = get_lowest_price(name_to_item_id[name], batch_dc)
